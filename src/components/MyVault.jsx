...
const MyVault = () => {
    ...
    return (
        <>
        <div className=" py-2 px-5 shadow-md border border-gray-300 flex items-center gap-3">
        ...
        <div className='flex flex-wrap justify-center sm:justify-normal sm:max-w-[85vw] max-h-[90vh] overflow-y-auto p-5 gap-5' >
            {storedPassword.map((item, index) => (
                <div onClick={() => { navigate(`/view/${item.id}`); dispatch(toggleAdd(false)) }}
                className='border border-gray-300 shadow-md p-6 min-w-60 max-w-75 h-40 justify-around flex flex-col items-center cursor-pointer hover:bg-yellow-100 transition-all duration-200' key={index}>
