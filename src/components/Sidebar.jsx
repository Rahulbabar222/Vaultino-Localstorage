...
const Sidebar = () => {
    ...
    return (
        <div className={`${sidebarHam === true ? "w-[100vw] flex flex-col " : "hidden"} sm:block sm:w-[25vw] xl:w-[15vw] h-[100vh] p-2 shadow-md sm:flex-col bg-linear-to-b from-neutral-500 to-neutral-900`}>
            <div className=' flex items-center justify-between'>
                <button disabled={addopen === true} onClick={() => { dispatch(toggleAdd(true)); dispatch(setSidebarHam(false)) }}
                    className='mx-3 my-2 py-1 px-4 bg-amber-300  hover:bg-amber-400 cursor-pointer disabled:bg-gray-400 w-fit rounded-full flex gap-4 justify-center items-center'>
            ...
            <button onClick={() => dispatch(setActiveComponent('MyVault'))} className='mx-5 my-2 flex gap-3 group w-fit'>
                <svg xmlns="http://www.w3.org/2000/svg" xmlSpace="preserve" id="lock" width="24" height="24" x="0" y="0" version="1.1" viewBox="0 0 512 512"
                    className={` group-hover:fill-amber-300 transition-colors duration-200 ${activeComponent === "MyVault" ? "fill-amber-300" : "fill-white"}`}>
                ...
                <span className={`text-lg group-hover:text-amber-300 transition-colors duration-200 ${activeComponent === "MyVault" ? "text-amber-300" : "text-white"}`}>
                    My Vault
                </span>
            </button>

            <button className='mt-15 flex flex-col items-center gap-5'>
                <span className='text-white'>More Features to be added..</span>
